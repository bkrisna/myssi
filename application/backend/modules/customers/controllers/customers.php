...
class Customers extends MY_Controller {
	...
	*/
	...
	public function create() {
        ...
        $item_data = array(
        	...
        	'custalias' => $items->custalias,
            'custaddr' => $items->custaddr,
			'custweb' => $items->custweb,
			'custphone' => $items->custphone,
			'custfax' => $items->custfax,
			'note' => $items->note
        );

	    $insert = $this->Customers_model->insert_entry($item_data);
	    if (intval($insert)) {
	        $item_data['id'] = $insert;
	        $data['success'] = true;
	        $data['total'] = 1;
	...
	*/
	...
	public function read() {
	    ...
	    $total_entries = $this->Customers_model->count_all_entries($filter);
	    $entries = $this->Customers_model->get_all_entries($filter, $limit, $offset, $order_by);
	...
	*/
	...
	public function update() {
	    ...
	    $insert = $this->Customers_model->update_entry($filter, $items);
	    if (intval($insert)) {
	        $data['success'] = true;
	        $data['total'] = 1;
	        $data['items'] = $item_data;
	...
	*/
	...
	public function destroy() {
	    ...
	    if ($this->Customers_model->delete_enry($filter)) {
	        $data['success'] = true;
	    } else {
	        $data['success'] = false;
	        $data['title'] = 'Fehler';
