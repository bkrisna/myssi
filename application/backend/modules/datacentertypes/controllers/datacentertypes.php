...
class Datacentertypes extends MY_Controller {

	function __construct() {
        parent::__construct();
        $this->load->model('DatacenterTypes_model');
	}
	...
	*/
	...
	public function create() {
	    ...
	    $insert = $this->DatacenterTypes_model->insert_entry($item_data);
	    if (intval($insert)) {
	        $item_data['id'] = $insert;
	        $data['success'] = true;
	        $data['total'] = 1;
	...
	*/
	...
	public function read() {
	    ...
	    $total_entries = $this->DatacenterTypes_model->count_all_entries($filter);
	    $entries = $this->DatacenterTypes_model->get_all_entries($filter, $limit, $offset, $order_by);
	...
	*/
	...
	public function update() {
	    ...
	    $insert = $this->DatacenterTypes_model->update_entry($filter, $items);
	    if (intval($insert)) {
	        $data['success'] = true;
	        $data['total'] = 1;
	        $data['items'] = $item_data;
	...
	*/
	...
	public function destroy() {
	    ...
	    if ($this->DatacenterTypes_model->delete_enry($filter)) {
	        $data['success'] = true;
	    } else {
	        $data['success'] = false;
	        $data['title'] = 'Error';
