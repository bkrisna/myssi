...
class Datacenters extends MY_Controller {
	...
	*/
	...
	public function create() {
        ...
        $item_data = array(
            'dcname' => $items->dcname,
        	'dcaddress' => $items->dcaddress,
            'dcstate' => $items->dcstate
        );

	    $insert = $this->Datacenters_model->insert_entry($item_data);
	    if (intval($insert)) {
	        $item_data['id'] = $insert;
	        $data['success'] = true;
	        $data['total'] = 1;
	...
	*/
	...
	public function read() {
	    ...
	    $total_entries = $this->Datacenters_model->count_all_entries($filter);
	    $entries = $this->Datacenters_model->get_all_entries($filter, $limit, $offset, $order_by);
	...
	*/
	...
	public function update() {
	    ...
	    $insert = $this->Datacenters_model->update_entry($filter, $items);
	    if (intval($insert)) {
	        $data['success'] = true;
	        $data['total'] = 1;
	        $data['items'] = $item_data;
	...
	*/
	...
	public function destroy() {
	    ...
	    if ($this->Datacenters_model->delete_enry($filter)) {
	        $data['success'] = true;
	    } else {
	        $data['success'] = false;
	        $data['title'] = 'Error';
