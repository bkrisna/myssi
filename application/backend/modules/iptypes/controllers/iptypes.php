...
class Iptypes extends MY_Controller {

	function __construct() {
        parent::__construct();
        $this->load->model('IpTypes_model');
	}
	...
	*/
	...
	public function create() {
	    ...
	    $insert = $this->IpTypes_model->insert_entry($item_data);
	    if (intval($insert)) {
	        $item_data['id'] = $insert;
	        $data['success'] = true;
	        $data['total'] = 1;
	...
	*/
	...
	public function read() {
	    ...
	    $total_entries = $this->IpTypes_model->count_all_entries($filter);
	    $entries = $this->IpTypes_model->get_all_entries($filter, $limit, $offset, $order_by);
	...
	*/
	...
	public function update() {
	    ...
	    $insert = $this->IpTypes_model->update_entry($filter, $items);
	    if (intval($insert)) {
	        $data['success'] = true;
	        $data['total'] = 1;
	        $data['items'] = $item_data;
	...
	*/
	...
	public function destroy() {
	    ...
	    if ($this->IpTypes_model->delete_enry($filter)) {
	        $data['success'] = true;
	    } else {
	        $data['success'] = false;
	        $data['title'] = 'Error';
